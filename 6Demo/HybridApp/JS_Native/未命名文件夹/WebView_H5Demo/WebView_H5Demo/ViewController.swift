...
class ViewController: UIViewController {
    ...
    @IBAction func toFirstVC(_ sender: UIButton) {
        ...
        if sender.tag == 200 {
            ...
            let third_webJSVC = storyboard?.instantiateViewController(withIdentifier: "ThirdParty_WebViewVC")
            navigationController?.pushViewController(third_webJSVC!, animated: true)
        }else {
            let third_wkwebJSVC = storyboard?.instantiateViewController(withIdentifier: "ThirdParty_WKWebViewVC")
            navigationController?.pushViewController(third_wkwebJSVC!, animated: true)
        }
